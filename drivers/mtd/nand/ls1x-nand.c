...
static int ls1x_nand_probe(struct platform_device *pdev)
{
	...
	mtd = kzalloc(sizeof(struct mtd_info) + sizeof(struct ls1x_nand_info),
			GFP_KERNEL);
	if (!mtd) {
...
static int __init ls1x_nand_init(void)
{
	return platform_driver_register(&ls1x_nand_driver);
}

static void __exit ls1x_nand_exit(void)
{
    platform_driver_unregister(&ls1x_nand_driver);
}

module_init(ls1x_nand_init);
module_exit(ls1x_nand_exit);
